...
# For each gene to observe, we get the number of cells expressing it for each cluster
sapply(obs_list, function(gene_name){
...
## Conclusion - `r if("1" %in% scenarios) "Scenario 1" else "Preparation"`
...
```{r scenario-1-early-stop, eval=scenarios == c("1")}
cat("# SessionInfo")
...
```{r regression-global, eval="2" %in% scenarios}
...
```{r regression-cycling, eval="3" %in% scenarios, fig.show='hold'}
