...
## Remove the unwanted cells based on feature expression

```{r filter-out-unwanted-cells}
filtered_list <- singlets
to_remove <- c()
if(!is.null(ff_list)){

  # Loop through each feature in ff_list
  for(i in 1:length(ff_list)){
    # Check if the feature is present in the RNA assay's var.features and print
    # message if not
    if(!ff_list[[i]] %in% singlets@assays[["RNA"]]@var.features){
      cat(paste0("No gene named ", ff_list[[i]], " found, skipping...\n"))
      to_remove <- append(to_remove, i)
    }
  }

  # Remove features that were not found in the data
  ff_list <- setdiff(ff_list, to_remove)

  # Fetch data for the selected features
  expr <- FetchData(object = singlets, vars = ff_list)

  # Filter out cells where expression of all selected features is 0
  filtered_list <- singlets[, which(FALSE == (expr > 0))]
}

# Export filtered_list to an RDS file
export_rds(filtered_list, paste0("04_", STEP_ID, "_goodQualityCells_", DATASET, ".rds"))
```

## Observe genes of interest 
...
### Genes of interest on the UMAP {.tabset .tabset-pills .tabset-fade}
```{r observe-genes-umap, result='asis', fig.align='default', fig.size='50%'}
invisible(lapply(obs_list, function(gene_name){
  gene_name = "Hba-a1"
  cells_gene_pos <- which(GetAssayData(filtered_list, slot="data")[gene_name,]>0)
  Idents(filtered_list) <- clustOfInterest
  FeaturePlot(fltd, features=gene_name, order=T)

  p <- DimPlot( filtered_list,
         reduction = "umap",
         cells.highlight = cells_gene_pos,
         cols.highlight = "#FF000088", 
         sizes.highlight = Seurat:::AutoPointSize(filtered_list)*4, 
...
### Genes of interest among clusters

print(df_obs)
```{r observe-genes-table}
sorted_clust_list <- sort(as.list(unique(filtered_list@meta.data[clustOfInterest]))[[1]])
df_obs <- data.frame("cluster"=sorted_clust_list)

for(gene_name in obs_list) {
  df_obs[[paste0(gene_name, "+_cells")]] <- sapply(sorted_clust_list, function(clust) {
    sum(GetAssayData(object = filtered_list, slot = "data")[gene_name,which(filtered_list@meta.data[clustOfInterest]==clust)]>0)
  })
}

df_obs$total_cells <- sapply(sorted_clust_list, function(clust) {length(which(filtered_list@meta.data[clustOfInterest]==clust))})
...
After removing the **`r doubletsNb`** doublet cells, we also reduced our
dataset from cells expressing the **`r paste( unlist(ff_list), collapse='', sep=",")`** genes. It is
equivalent to the removal of
**`r dim(singlets)[2] - dim(filtered_list)[2]`** unwanted cells
from the singlets. Now, the scenario 1 dataset contains **`r dim(filtered_list)[2]`**
cells. It is ready for the downstream analysis.

```{r cleaning, include=FALSE}
rm(filtered_list, doubletsNb, fltd)
invisible(gc())
```
...
## Remove unwanted cells - scenarios 2 and 3

```{r filter-out-unwanted-cells-2, results='asis'}
...
cat(paste0("
On the global cell cycle regressed dataset, we removed ", dim(singlets)[2] - dim(filtered_list)[2], " 
cells, as they were expressing the **", paste( unlist(ff_list), collapse='', sep=','), ",** gene marker.  
The dataset has ", dim(filtered_list)[2], " remaining cells.
"))
rm(singlets, globalReg, filtered_list)
invisible(gc())
...
<<filter-out-unwanted-cells>>
cat(paste0("
On the cycling phases regressed dataset, we removed ", dim(singlets)[2] - dim(filtered_list)[2], " 
cells.  
The dataset has ", dim(filtered_list)[2], " remaining cells.
"))
rm(singlets, cyclingReg, filtered_list)
invisible(gc())
```
