...
After quality control, we still have to perform doublets removal and decide
if we want to regress out the cell cycle effect. We propose two cell cycle
regression strategies:

1. a regression across all the cell cycle phases, that we call
*global regression*, and

2. a regression applied only to cycling cells (ie. G2/M and S phases), which
allows keeping the cycling and non-cycling signal. We call this strategy
*cycling regression*.

Overall, we have now the possibility to use 3 scenarii:

- scenario 1: remove doublets only

- scenario 2: remove doublets and apply global regression

- scenario 3: remove doublets and apply cycling regression

As the scenario 1 corresponds to the first step of the scenarios 2 and 3, we
simply reuse the output from the scenario 1 to perform the cell cycle regressions
(scenarios 2 and 3).
...
cat("## Cell annotations through label transfer from the atlas \n")
```
...
## Doublet identifications - DoubletFinder run

We next compared the identified doublets with UMI counts.
...
To further explore the repartition of the identified doublets, we provide the number of doublets identified in each cluster or each cell type. Note that cell types are obtained here from label transfer from the atlas. 
...
We removed the **`r doubletsNb`** doublet cells.
...
# Cell cycle score assignations


## Cell cycle markers

Cell cycle genes list has been updated in 2019 (REF). Cell cycle markers are only
for the cycling phases, G2/M and S. If cells have few chance to be in those
cycling phases based on the markers, they are considered to be in the G1 phase.
...
estimated to be in the S phase. Subsequently, any cell having a value of `G2M.Score > 0`
is estimated to be in the G2M phase. All the other cells are estimated to be in the G1
phase.  
However, depending on the datasets, the default thresholds might not be adapted. It is possible to
set up new thresholds manually to determine the limits between S and G1 phase and G2M and 
G1 phase.

```{r cc-assign-score-2, fig.show='hold'}
# MANUAL PHASE ASSIGNATION BASED ON SCORE THRESHOLDS
...
Next, to visualize cell variability according to the cell cycle phases (before cell cycle regression), we apply PCA using only the cell cycle genes. The cells here segregate according to their estimated cell cycle phase.
...
We apply regression of cell cycle (using phase scores) following the scenario number 2. After regression, the cells do not seeggregate according to the cell cycle in a PCA using cell cycle genes.
")
...
We apply regression of cell cycle (using phase scores of cycling cells) following the scenario number 3. After regression, the cycling cells (G2/M and S cells) do not seggregate in a PCA using cell cycle genes, but these cells are separated from G1 cells.
")
...
# Preliminary study of a few number of selected genes

In order to have a first exploration of the dataset, we provide here the code to visualize the expression of a few number of selected genes. For each gene is presented both a feature plot of its expression as well as all cells expressing it. 
For instance, this could be used to remove some clusters based on the expression of one specific gene in the cells of the cluster.

If we want to remove some clusters based on the analysis done below, the clusters to be removed will need to be specified before the re-application of the preprocessing pipeline, to be done after this step 4 "doublet removal, cell cycle regression and gene overview" 



## Feature Plots of specific genes {.tabset .tabset-pills .tabset-fade}
...
## Presence of specific genes in the cluster cells
...
knitr::kable(df_obs, caption = "Number of cells expressing the specific gene in each cluster") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```

# Next step

Here, we finished the preparation of the dataset.
