...
invisible(lapply(obs_list), function(gene_name){
  ...
  Idents(filtered_list)
  FeaturePlot(fltd, features="Hba-a1", order=T)
...
sorted_clust_list <- sort(unique(filtered_list[clustOfInterest]))
df_obs <- data.frame("cluster"=sorted_clust_list)

lapply(obs_list, function(gene_name) {
  df_obs[[paste0(gene_name, "+_cells")]] <- sapply(sorted_clust_list, function(clust) {
    sum(GetAssayData(object = filtered_list, slot = "data")[gene_name,filtered_list@meta.data[clustOfInterest]==clust]>0)
  })
})
