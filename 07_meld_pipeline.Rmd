...
knitr::opts_chunk$set(
  ...
  # plots
  fig.path = paste0(PATH_OUT_FIG, "/07_MELD"),      # is set later, in chunk setup2
...
if(!file.exists(gsub(".rds",".h5Seurat", input_file))){
  convert(input_file)
}
```
...
adata = sc.read_h5ad(Path(r.input_file).with_extension("h5ad"))
...
def replicate_normalize_densities(sample_densities, replicate):
    ...
    if replicate is None:
      samples_likelihoods = sklearn.preprocessing.normalize(sample_densities, norm="l1")
    else:
...
sample_likelihoods = replicate_normalize_densities(sample_densities, replicates_markers)
```
...
pred = pd.read_csv(Path(PATH_ROOT, "07_annotDEAviz", f"transferAnnotation_combinedData_{r.DATASET}.csv"), header=0)
clusters = pd.read_csv(Path(PATH_ROOT, "08_combineData", f"clustering_{r.DATASET}_scenario.{r.scenario}_meth.{r.CLUST_METH}_res.{r.CLUST_RES}.csv"), header=0)
colortable = pd.read_csv(Path(PATH_ATLAS, "colorsSheet.csv"), header=0)
...
cluster_colors = colormaps["tab20"].resampled(len(np.unique(metadata["clusters"])))
```

```{python plot-likelihoods}
...
scprep.plot.scatter2d(umap, c=metadata["prediction"], cmap=celltype_colors,
                      title='Celltypess visualisation', ticks=False, ax=axes[1], legend=False, legend_anchor=(0,2), legend_loc="upper center") # Legend is finicky, will either try to correctly display or delete the chunk
