...
# Load `r DATASET` data for scenario `r scenario`
...
# Density distribution on the UMAP

```{python plot-meld, results='asis', fig.align='default'}
# Display the density of the samples on the umap.
for i in range(len(sample_densities.columns)):
...
```{python plot-likelihoods, results='asis', fig.align='default'}
...
if(len(experimental_samples) > 1):
    ...
    for curr_sample in experimental_samples:
        ...
        scprep.plot.scatter2d(umap, c=sample_likelihoods[curr_sample], cmap=meld.get_meld_cmap(),
                            vmin=vmin, vmax=vmax,
                            title=f"{curr_sample} gigh contrast", ticks=False)
        plt.tight_layout()
        plt.show()
else:
...
print("## Replicates mean and standard deviation")
...
```{python clustering, fig.align='default'}
#fig, axes = plt.subplots(1,3, figsize=(16,8))

scprep.plot.scatter2d(umap, c=metadata["clusters"], cmap=cluster_colors,
                      title='Cluster visualisation', ticks=False, legend=False, legend_anchor=(0,2), legend_loc="upper center") # Legend is finicky, will either try to correctly display or delete the chunk
plt.tight_layout()
plt.show()

scprep.plot.scatter2d(umap, c=metadata["prediction"], cmap=celltype_colors,
                      title='Celltypes visualisation', ticks=False, legend=False, legend_anchor=(0,2), legend_loc="upper center") # Legend is finicky, will either try to correctly display or delete the chunk
plt.tight_layout()
plt.show()

scprep.plot.scatter2d(umap, c=metadata["cond_likelihood"], cmap=meld.get_meld_cmap(),
                        vmin=0, vmax=1,
                        title=f"MELD ({conditions[0]} likelihood)", ticks=False)
plt.tight_layout()
plt.show()
plt.clf()
```
...
for clustering in ["clusters","prediction"]:
  ...
  plt.title(f"Likelihood of the {'cluster' if clustering == 'clusters' else 'celltype'} cells.")
  plt.tight_layout()
  plt.show()
plt.clf()
```