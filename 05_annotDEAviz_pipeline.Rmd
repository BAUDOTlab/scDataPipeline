...
cat("Each cell has been annotated through label transfer from the atlas\n")
```
...
The goal here is to compute the differential expression between a given cluster and all but this given cluster remaining clusters (each versus rest). This will allow identifying the features significantly over or under expressed in a given cluster. The top x most significant features will be considered as biomarkers (top 5 to top 20 depending on the analyses). These significantly deregulated features can then be used as cluster biomarkers for the manual annotation of cluster type.
To perform DEA, we implemented pseudo-bulking of pseudo-replicates. Three pseudo-replicates are computed from the given cluster and the rest of the clusters. DESeq2 is then applied on these 3 versus 3 pseudo-bulk datasets. Top 20 most differentially expressed features are given for each cluster.
...
### DotPlot of the biomarkers on cluster identity
...
### FeaturePlots of the biomarkers {.tabset .tabset-pills .tabset-fade}
...
### ViolinPlots of the biomarkers {.tabset .tabset-pills .tabset-fade}
...
cat("Cells have been annotated through label transfer from the atlas")
...
cat("### UMAP of the celltype identity {.tabset .tabset-pills .tabset-fade}")
## UMAP the celltype
DimPlot(fltd,
		reduction = "umap",
        pt.size = 1,
...
cat("### DotPlot of the biomarkers in celltypes")
...
cat("### DotPlot of the genes of interest in celltypes")
