...
```{r echo=FALSE, eval=exists("external_gene_name")}
...
clustOfInterest <- grep(paste0(DefaultAssay(renamed), "_snn_res.", clust_res), names(renamed@meta.data), value = TRUE)
...
```{r addPlots-4}
Idents(fltd) <- factor(renamed@meta.data[, colname], levels = sort(levels(as.factor(renamed@meta.data[, colname]))))
```
