...
```{r, results='asis', eval=(gn_col == 1), echo=(gn_col==1)}
cat("""
...
", setdiff(ensembl.ids, geneNames108$ensembl_gene_id), "
...
```{r RenameGenesSeurat,eval=(gn_col == 1), echo=(gn_col==1)}
...
clustOfInterest <- grep(paste0("RNA_snn_res.", clust_res), names(fltd@meta.data), value = TRUE)
...
```{r addPlots-1, eval=!is.null(genes_of_interest), echo=!is.null(genes_of_interest)}
  # Load list of genes of interest
  goi <- read.table(file.path(PATH_REQUIREMENTS, genes_of_interest))
  goiList <- goi$V1
...
```{r cluster-dotPlot-goi, eval=!is.null(genes_of_interest), echo=!is.null(genes_of_interest)}
...
```{r cluster-featurePlot-goi, out.width='20%', fig.align='default', eval=!is.null(genes_of_interest), echo=!is.null(genes_of_interest)}
...
```{r addPlots-4}
Idents(fltd) <- factor(fltd@meta.data$celltype_singleData, levels = sort(levels(as.factor(fltd@meta.data$celltype_singleData))))
```
...
cellsData <- data.frame(fltd@reductions[["umap"]]@cell.embeddings, fltd@meta.data$celltype_singleData)
colnames(cellsData) <- c(colnames(fltd@reductions[["umap"]]@cell.embeddings), "celltype")

invisible(sapply(levels(as.factor(fltd@meta.data$celltype_singleData)), function(act)
{
    cat("#### ", act, "\n")
    ...
    geom_point( data = cellsData[cellsData$celltype == act,], # Now provide data including column for facetting
                ...
                size  = 3.5) +
    blankTheme() +
    scale_fill_manual(values = colors.celltype[fltd@meta.data$celltype_singleData], aesthetics = "color") +
    NoLegend() +
    ggtitle(paste0(act))
    )
...
```{r celltype-dotPlot-goi, eval=!is.null(genes_of_interest), echo=!is.null(genes_of_interest)}
