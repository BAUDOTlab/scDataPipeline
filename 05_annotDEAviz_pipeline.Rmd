...
colors.table <- read.csv("../00_rawData/Li_et_al_Atlas/colorsSheet.csv", header=T, comment.char="", as.is=T)
...
```{r addPlots-4}
Idents(renamed) <- factor(renamed@meta.data$celltype_singleData, levels = sort(levels(as.factor(renamed@meta.data$celltype_singleData))))
```
...
cellsData <- data.frame(renamed@reductions[["umap"]]@cell.embeddings, renamed@meta.data$celltype_singleData)
colnames(cellsData) <- c(colnames(renamed@reductions[["umap"]]@cell.embeddings), "celltype")

invisible(sapply(levels(as.factor(renamed@meta.data$celltype_singleData)), function(act)
{
    cat("#### ", act, "\n")
    ...
    geom_point( data = cellsData[cellsData$celltype == act,], # Now provide data including column for facetting
                ...
                size  = 3.5) +
    blankTheme() +
    scale_fill_manual(values = colors.celltype[renamed@meta.data$celltype_singleData], aesthetics = "color") +
    NoLegend() +
    ggtitle(paste0(act))
    )
