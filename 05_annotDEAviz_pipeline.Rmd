...
```{r variableDef1-1, include=FALSE, eval=combinedD}
# PATHS
...
```{r variableDef1-2, include=FALSE, eval=!combinedD}
# PATHS
...
```{r, results='asis', eval=(gn_col == 1 && atlas), echo=(gn_col==1)}
cat("
...
```{r, eval=(gn_col == 1 && atlas), results='asis'}
cat(paste0("
...
```{r echo=FALSE, eval=atlas && dim(duplicatesGN)[1] !=0, results='asis'}
cat("
...
```{r RenameGenesSeurat, eval=(gn_col == 1 && atlas), echo=(gn_col==1)}
# Remove ensembl gene IDs with multiple match to gene names
geneNamesUniq <- geneNames108[-which(geneNames108$ensembl_gene_id %in% ifelse(dim(duplicatesGN)[1] !=0, duplicatesGN$ensembl_gene_id, geneNames108$ensembl_gene_id)),]
...
`r if(atlas){cat("## Celltype identity {.tabset .tabset-pills .tabset-fade}")}`

```{r addPlots-4, eval=atlas}
Idents(fltd) <- factor(fltd@meta.data[, colname], levels = sort(levels(as.factor(fltd@meta.data[, colname]))))
```

`r if(atlas){cat("### DimPlot of the celltype identity {.tabset .tabset-pills .tabset-fade}")}`
...
`r if(atlas){cat("### DotPlot of the markers on celltype identity")}`
...
`r if(atlas){cat("### DotPlot of the genes of interest on celltype identity")}`
