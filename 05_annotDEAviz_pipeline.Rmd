...
```{r addPlots-4}
Idents(renamed) <- factor(renamed@meta.data[, colname], levels = sort(levels(as.factor(renamed@meta.data[, colname]))))
```
...
cellsData <- data.frame(renamed@reductions[["umap"]]@cell.embeddings, renamed@meta.data[, colname])
colnames(cellsData) <- c(colnames(renamed@reductions[["umap"]]@cell.embeddings), "celltype")

invisible(sapply(levels(as.factor(renamed@meta.data[, colname])), function(act)
{
    cat("#### ", act, "\n")
    ...
    geom_point( data = cellsData[cellsData$celltype == act,], # Now provide data including column for facetting
                ...
                size  = 3.5) +
    blankTheme() +
    scale_fill_manual(values = colors.celltype[renamed@meta.data[, colname]], aesthetics = "color") +
    NoLegend() +
    ggtitle(paste0(act))
    )
