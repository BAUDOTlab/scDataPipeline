...
```{r run-dea}
#Set the maximum amount of data able to be passed to future workers at 850Mb
...
markers.sign <- markers[markers$p_val_adj < padj_threshold & markers$avg_logFC > fc_threshold, ]
...
markers.sign$gene_name <- corr_table$GeneName[match(markers.sign$gene, corr_table$ENSid)]
markers.sign <- markers.sign[ , c(8, 6, 2:4, 1, 5, 7)]

write.table(x = markers.sign, file = file.path(PATH_OUT_ANALYSIS,
                                               paste0("Wilcoxon_markers_min_fc_", fc_threshold, "_pad_", padj_threshold, "_",
                                               DATASET, "_scenario.", scenario, "_res.", clust_res, ".csv")), sep = ",", row.names = F, col.names = T)
...
# write.table(x = topNmarkers, file = file.path(PATH_OUT_ANALYSIS,
#                                               paste0("DESeq2_topmarkers", scenario, "_res.", clust_res, ".csv")), sep = ",", row.names = F, col.names = T)
write.table(x = topNmarkers, file = file.path(PATH_OUT_ANALYSIS,
                                              paste0("Wilcoxon_topmarkers", scenario, "_res.", clust_res, ".csv")), sep = ",", row.names = F, col.names = T)
