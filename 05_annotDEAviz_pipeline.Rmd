...
```{r write-ensemblID-mapping-section, results='asis', eval=FALSE, echo=FALSE}
cat("
...
```{r reduce-gene-list, eval=FALSE, results='asis'}
cat(paste0("
...
```{r table-duplicateGN, echo=FALSE, eval=FALSE, results='asis'}
cat("
...
```{r rename-genes-seurat, eval=FALSE, echo=FALSE}
...
markers <- FindAllMarkers2(fltd, method="deseq", min_fc = 0.1)
#markers.sign <- markers[markers$p_val_adj < 0.001 & markers$avg_logFC > log(1.5), ]

# Add gene names
markers$gene_name <- corr_table$GeneName[match(markers$gene, corr_table$ENSid)]
...
markers.sign <- markers.sign[ , c(6, 8, 2:4, 1, 5, 7)]
...
markers$group1 <- as.numeric(markers$group1)
topNmarkers <- extract_top_features(markers, topn = top_markers)
datatable(topNmarkers, rownames = FALSE, filter = "top", options = list(pageLength = 5, scrollX=T))
...
write.table(x = topNmarkers, file = file.path(PATH_OUT_ANALYSIS,
                                              paste0("DESeq2_", scenario, "_res.", clust_res, ".csv")), sep = ",", row.names = F, col.names = T)

```

# DE plots
```{r de-plots}
markers$comp <- paste0("Cluster ", markers$group1)
markers$comp <- factor(markers$comp, levels = unique(markers$comp))
ggplot(markers, aes(log_fc, -log10(padj))) +
