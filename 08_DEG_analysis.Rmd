...
file_names <- list.files(path = file.path(PATH_ROOT, "09_DEG_analysis"), pattern = "^cluster_(\\d+)_DEG_.*\\.csv$", full.names = TRUE)

# Define the path to the folder containing the input files
input_folder <- file.path(PATH_ROOT, "09_DEG_analysis")

# Create the output parent folder if it doesn't exist
output_parent_folder <- file.path(PATH_ROOT, "10_fgsea_analysis")
dir.create(output_parent_folder, showWarnings = FALSE)
