...
knitr::opts_chunk$set(
  ...
  # plots
  fig.path = paste0(PATH_OUT_FIG, "/copy_08_GSEA_scenario_", scenario, "_"),
...
library(devtools)
devtools::install_local("../01_requirements/DElegate-main.zip")
library(DElegate)
...
perform_deg <- function(clust) {
    ...
    if (length(table(cluster_data$orig.ident)) > 1 & as.integer(table(cluster_data$orig.ident)[1]) >= 30 & as.integer(table(cluster_data$orig.ident)[2]) >= 30) {
        ...
        options(future.globals.maxSize = 3.5*1024*1000000)
        set.seed(general_seed)
        de_results <- findDE(object = cluster_data, method = "deseq", cluster = clust)
        ...
        **Top 20 genes in term of logFC (purple)** :
        ...
        invisible(write.table(de_results[, c(11, seq(2,10), 1)], file =file.path(PATH_OUT_ANALYSIS, paste0("copy_cluster_", clust, "_DEG_", levels(de_results$group1), "_vs_", levels(de_results$group2), ".csv")), col.names = TRUE, row.names = FALSE, sep = ","))
...
# Get only files starting with "copy"
file_names <- file_names[grep("copy", file_names)]
...
perform_fgsea_analysis <- function(csv_path) {
  ...
  for (db in databases) {
    ...
    if (nrow(fgseaRes) == 0) {
        ...
        output_file <- file.path(output_folder, paste0("copy_fgseaRes_", tools::file_path_sans_ext(basename(csv_path)), "_", db, ".csv"))
