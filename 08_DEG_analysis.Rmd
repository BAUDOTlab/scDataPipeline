...
perform_fgsea_analysis <- function(csv_path) {
  ...
  for (db in databases) {
    ...
    if (nrow(fgseaRes) == 0) {
        ...
        fgseaRes <- fgseaRes %>% 
                    separate(pathway, c("database","newx"), sep="_", remove=F, extra="merge")
        fgseaRes$newx <- gsub("_", " ", fgseaRes$newx)
        ...
        fgseaRes <- fgseaRes %>%
            ...
            mutate(upreg = ifelse(NES > 0,"1","0"))
        
        print(ggplot(fgseaRes, aes(x = NES, y = newx, color = upreg, size = GeneRatio)) +
            geom_point(aesthetics = "size") +
            geom_segment(aes(x = 0, y = newx, xend = NES, yend = newx), size = 0.4) +
            scale_fill_manual(values = c("1"="red","0"="blue"), aesthetics = "color") +
            scale_size_continuous(range = c(3,13)) +
            scale_y_discrete(labels = label_wrap(50)) +
            guides(
              color = "none",
              size = guide_legend(position = c(0.25,0.87))
              ) +
            labs(y = "Pathways", x = "Normalized Enrichment Score",
