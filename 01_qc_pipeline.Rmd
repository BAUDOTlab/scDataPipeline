...
knitr::opts_chunk$set(
  ...
  # figures
  fig.path = paste0(PATH_OUT_FIG, "/01_qc_")
)
```
...
```{r add-metadata}
if (grepl("fibro", DATASET)) {
    localization <- "Fibroblast"
} else {
    localization <- "Cardiomyocyte"
}
...
SO <- AddMetaData(object = SO, metadata = localization,    col.name = "localization")
SO <- AddMetaData(object = SO, metadata = condition, col.name = "condition")
...
cellnames <- paste(localization, condition, UMIs, sep = "_")
...
write.table(
    ...
    file = file.path(
        PATH_OUT_ANALYSIS, 
        paste0("cellsList_filteredOut_", DATASET, "_mitoLow.", mito_low, "_mitoHigh.", mito_high, ".txt")), 
    row.names = FALSE, 
    col.names = FALSE
)
```
...
SO@meta.data[SO@meta.data$percent.ribo >= ribo_low, "outlier.ribo"] <- FALSE
...
write.table(
    ...
    file = file.path(
        PATH_OUT_ANALYSIS,
        paste0("cellsList_filteredOut_", DATASET, "_ribo.", ribo_low, ".txt")),
    row.names = FALSE, 
    col.names = FALSE
)
```
...
write.table(
    ...
    file = file.path(
        PATH_OUT_ANALYSIS,
        paste0("cellsList_filteredOut_", DATASET, "_UMIcounts_l.", min_counts, "_h.", max_counts, ".txt")),
    row.names = FALSE,
    col.names = FALSE
)
```
...
SO@meta.data[SO@meta.data$nFeature_RNA >= min_feat, "outlier.nFeature"] <- FALSE
...
write.table(
    ...
    file = file.path(
        PATH_OUT_ANALYSIS,
        paste0("cellsList_filteredOut_", DATASET, "_featureCounts.", min_feat, ".txt")),
    row.names = FALSE,
    col.names = FALSE
)
```
...
In addition, features that have been detected in `r min_cells` or less will also be removed.
It involves `r table(df_non0$Nbr_of_cells <=  min_cells)["TRUE"]`.

In total, `r table(df$Nbr_of_cells <=  min_cells)["TRUE"]` features will be removed when the quality control is applied.
...
write.table(
    ...
    file.path(
        PATH_OUT_ANALYSIS, 
        paste0("cellsList_filteredOut_", DATASET, "_all.txt")),
    row.names = FALSE,
    col.names = FALSE
)
...
We export the *cellIDs*, *sample*, *localization*, *condition*, *percent.mito*,
*outlier.mito*, *percent.ribo*, *outlier.ribo*, *outlier.nCount* and
*outlier.nFeature* metadata for the filtered and complete dataset. We then
reduce the metadata table to the *orig.ident*, *nCount_RNA* and *nFeature_RNA*
metadata and save the filtered and complete objects.
