...
if (goodQ){
    title_var <- paste0("|
    | Step 5: Preprocessing cleaned data -- ", DATASET)
} else if (combinedD) {
    title_var <- paste0("|
    | Step 8: Preprocessing the ", ifelse(combine_meth=="merge", "merged", "integrated"), " data -- ", DATASET)
} else {
    title_var <- paste0("|
...
# We might want to keep all cells and just flag those that are not of interest
...
cells_to_keep <- which(SO@meta.data[clustOfInterest] != rm_clust)
...
```{r exports}
# variable features
saveHVG <- function(var.feats.dataset) {
    if (!goodQ) {
        write.table(
            var.feats.dataset,
            file = file.path(
...
invisible(saveHVG(var.feats.list))
