...
```{r load-data-1, eval=!goodQ, echo=!goodQ}
...
process <- function(SO, scenario) {
    ...
    if (goodQ) {
        saveRDS(SO, file=file.path(PATH_RDS_OBJECTS, paste0("05_prepocessed_", DATASET, "_scenario.", scenario, ".rds")))
    } else {
        saveRDS(SO, file=file.path(PATH_RDS_OBJECTS, paste0("02_prepocessed_", DATASET, "_", FILTER, ".rds")))
    }
...
saveHVG <- function(var.feats.dataset, scenario) {
    if (!goodQ) {
        write.table(
            ...
            file = file.path(
                PATH_OUT_ANALYSIS,
                paste0("varFeatures_meth.", hvg_meth, "_nb.", hvg_num, "_", DATASET, "_", ifelse(!goodQ, FILTER, paste0("scenario.", scenario)), ".txt")),
            row.names = FALSE,
            col.names = FALSE
        )
    }
...
saveDR <- function(SO, scenario) {
    invisible(sapply(c("pca", "umap"), function(dr) {
        export_dimred(SO, dr, paste0("DR_", dr, "_", DATASET, "_", ifelse(!goodQ, FILTER, paste0("scenario.", scenario)), ".csv"))
    }))
}
...
if (goodQ) {
    defaultMDnames <- c(defaultMDnames, "tissue_DF")
}
...
saveCluster <- function(SO, scenario) {
    export_metadata(
        ...
        paste0(
            ...
            ifelse(!goodQ, FILTER, paste0("scenario.", scenario)),
            "_meth.", clust_meth,
            "_res.", clust_res,
            ".csv"))
}
